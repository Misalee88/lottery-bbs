...
public class NumberUtils {
	...
	public static String formatNumber(Number number, String format) {
		...
		if ("rate".equals(format)) {
			return format(number.doubleValue() / 10, 1).toString().replace("\\.0", "") + "折";
		}
		int scale = -1;
		...
		try {
			...
			value = num.setScale(scale, BigDecimal.ROUND_HALF_UP).toPlainString();
	 ...
	 * @return
	 */
	public static double ceil(double num,double norm){
		return Math.ceil(num/norm*100)/100;
	}
