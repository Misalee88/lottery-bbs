...
public class StringUtils {
	...
	public static List<String> find(String input, String regex) {
		...
		Matcher matcher = Pattern.compile(regex, Pattern.CASE_INSENSITIVE | Pattern.DOTALL).matcher(input);
	...
	public static String trans(String str, String charset, String charsetFrom) {
		...
		try {
			byte[] bs = charsetFrom != null ? str.getBytes(charsetFrom) : str.getBytes();
	...
	public static byte[] hexToBytes(String hex) {
		...
		for (int i = 0; i < length; i++) {
			d[i] = (byte) Integer.parseInt(hex.substring(2 * i, 2 * (i + 1)), 16);
		}
		return d;
	}

	/**
	 *  判断字符串str是否为空，为空返回target
	 * @param str
	 * @param target
	 * @return
	 */
	public static String checkNull2Target(String str,String target){
		return org.springframework.util.StringUtils.isEmpty(str) ? target : str;
	}

	/**
	 *  截取字符串中的字母 去重后组成map
	 *  eg :  1<abc-cdf<2  截取出数组[{abc:abc},{cdf:cdf}]
	 * @param chars
	 * @return
	 */
	public static Map<String,String> char2array(String chars){
		String pattern = "[^a-zA-Z]";

		String[] maths =  Pattern.compile(pattern).split(chars);
